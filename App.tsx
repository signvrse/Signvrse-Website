...
import { 
  ArrowRight, PlayCircle, Users, Globe, Shield, 
  Cpu, Sparkles, MessageSquare, Check, Menu, Plus, Minus, ScanFace, Send, Building2, Ear, Star, Quote, Mail
} from 'lucide-react';
import TextVideoHover from './components/TextVideoHover';
import { sendContactEmail } from './lib/api';

export type ViewState = 'LANDING' | 'DEMO' | 'ABOUT' | 'WHAT_WE_DO' | 'CASE_STUDIES' | 'PARTNERSHIPS' | 'CAREERS' | 'CONTACT' | 'PRODUCTS' | 'PRIVACY' | 'TERMS' | 'BLOG';
...
function App() {
  ...
  const renderView = () => {
    const commonProps = {
        ...
        onLaunchDemo: () => setView('DEMO'),
    };

    if (view === 'ABOUT') return <AboutPage {...commonProps} />;
    if (view === 'WHAT_WE_DO') return <WhatWeDoPage {...commonProps} />;
    if (view === 'CASE_STUDIES') return <CaseStudiesPage {...commonProps} />;
    if (view === 'PARTNERSHIPS') return <PartnershipsPage {...commonProps} />;
    if (view === 'CAREERS') return <CareersPage {...commonProps} />;
    if (view === 'CONTACT') return <ContactPage {...commonProps} />;
    if (view === 'PRODUCTS') return <ProductsPage {...commonProps} />;
    if (view === 'PRIVACY') return <PrivacyPolicy {...commonProps} />;
    if (view === 'TERMS') return <TermsOfService {...commonProps} />;
    if (view === 'BLOG') return <BlogPage {...commonProps} />;
    
    // Default Landing Page content
    return (
      <div
        id="main-content"
        className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50 font-sans selection:bg-brand-100 selection:text-brand-900 transition-colors duration-300"
      >
        {/* Navigation */}
